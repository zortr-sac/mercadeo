...
Casi todas las entidades llevan `businessId: string | null`. Un `businessId` concreto = contenido propio de ese negocio; `null` = contenido global de plataforma visible para todos. Los filtros de repositorio aplican el patrón `item.businessId === filter.businessId || item.businessId === null`. Respeta este alcance al agregar queries o seeds.

## Auth y autorización

- **Auth real (Supabase)**: `src/lib/session.ts` resuelve la sesión vía `supabase.auth.getUser()` y carga el perfil (modo mock: cookie demo `nexo_mentor_session`). `getSession()`, `requireSession()`, `requireRole(min)` son `server-only` y redirigen si no cumplen. El login conserva el UX de "elegir perfil" pero hace `signInWithPassword` real contra usuarios sembrados (`src/app/(auth)/login/actions.ts`). **Password demo compartido: `NexoDemo2026!`** (admin@nexomentor.app, carla@bienestarandino.com, sofia@example.com, luis@example.com). El RPC `list_demo_users()` (anon) alimenta la pantalla de login. `src/proxy.ts` refresca la sesión Supabase y protege rutas.
- **RBAC** (`src/lib/rbac.ts`): tres roles con jerarquía `member(0) < leader(1) < admin(2)`. `can(role, action)` contra una matriz de acciones; `hasAtLeast(role, min)`. Nota de naming: en este SaaS `leader` = "Admin de negocio" y `admin` = "Admin de plataforma".
- **Defensa en capas**: el layout `src/app/(app)/layout.tsx` exige `requireSession()`; páginas admin exigen `requireRole("admin")`. La UI condicional por rol es solo UX — la autorización real va en servidor (y será RLS). Lee `docs/security/security-model.md`, que es la fuente de verdad de seguridad y debe mantenerse consistente con `supabase/migrations/0001_security_baseline.sql`.
